...
export class BetSlipComponent implements OnDestroy {
  ...
  constructor(public dataService: DataService, private fb: FormBuilder) {
    ...
    this.addOrDeleteBallSuscription =
      this.dataService.addOrDeleteBallEmitter$.subscribe((ball) => {
  ...
  public ballsSelected(): Array<BallI> {
    let balls_selected = this.dataService.bet.drum.filter(
      (ball) => ball.isSelected
    );
    ...
    if (this.dataService.balls_selected > 0) {
      if (this.dataService.bet.input_stake) {
        if (this.dataService.bet.input_stake >= 5) {
          if (
            this.dataService.user.wallet >=
            this.dataService.bet.input_stake * this.dataService.balls_selected
          ) {
            this.errorMessage = '';
            this.dataService.total_money =
              this.dataService.bet.input_stake *
              this.dataService.balls_selected;
